...
const getEmpresasDepartamentos = async (req,res)  => {
    const response = await pool.query('SELECT DISTINCT e.* FROM empresa e INNER JOIN '+
    'espacios es ON es.id_empresa = e.id INNER JOIN tipo_espacio te ON te.id = es.id_tipo WHERE es.id_estado = 1 and te.id = 2')
    res.status(200).json(response.rows);
}

const getEmpresasSalones = async (req,res)  => {
    const response = await pool.query('SELECT DISTINCT e.* FROM empresa e INNER JOIN '+
    'espacios es ON es.id_empresa = e.id INNER JOIN tipo_espacio te ON te.id = es.id_tipo WHERE es.id_estado = 1 and te.id = 3')
    res.status(200).json(response.rows);
}
